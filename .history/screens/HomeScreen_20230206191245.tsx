...
const Products: React.FC<prodmin> = (props)=>{
  ...
  const {item} = props
  return(
    <View style={{marginBottom:2, width:'100%',borderRadius:10}}>
      <BlurView
        intensity={90}
        tint='light'
        style={{
      ...
      >
        ...
        <Text style={{fontFamily:'Josephin',fontWeight:'600'}}>{item.name}</Text>
        
        {(item.countInStock > 2) ?(<><Text style={{textAlign:'center',fontWeight:'800', color:'#3CB043'}}><Ionicons name='information-circle-outline' size={20} color='#3CB043'/> In Stock </Text></> ):(<><Text style={{textAlign:'center', color:'#B90E0A'}}><Ionicons name='information-circle-outline' size={20} color='#B90E0A'/> Out Of Stock </Text></>)}
      </BlurView>
    </View>
  )
}
