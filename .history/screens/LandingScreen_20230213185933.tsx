...
const _LandingScreen: React.FC<LandingProps> = (props)=>{
    ...
    const getLocation = async ()=>{
        ...
        if(status !== 'granted'){
            setErrorMsg('Permission tp access location is not granted')
            alert('Permission not granted')
            getPermissions()
        }
        console.log('heello')
        const location = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.Highest, maximumAge: 10000, timeout: 5000 })
        console.log('heey')
