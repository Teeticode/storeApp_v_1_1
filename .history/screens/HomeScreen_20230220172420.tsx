...
import { Categories,HomeCategory, SearchField } from '../components'
...
const Products: React.FC<prodmin> = (props)=>{
  ...
  const {item} = props
  return(
    <TouchableOpacity onPress={()=>{props.navigate('Product')}} style={{ width:'100%',borderRadius:10}}>
      <BlurView
        intensity={90}
        tint='light'
        style={{
              ...
              (productsReducer.availability.length >= 1)?(
                <>
                {
                  ...
                  >
                    ...
                    <FlatList
                      data={(productsReducer.availability).filter((prod)=>{
                        if(prod.category.name=== active){
                          prod
                        }else if(active ==='all'){
                          prod
                        }
